...
const ComicPage = ({ newBooksData, popularBooksData, evaluationBooksData }: any) => {
  ...
  return (
    <div className={styles.overall}>
      <Head>
        <title>Book Talk ｜ 小説・エッセイ</title>
        <meta name="viewport" content="width=device-width,initial-scale=1.0" />
        <meta name="description" content={"あなたの探したい本が見つかるBookTalk"} />
        <meta property="og:url" content={"https://booktalk.vercel.app/novelBooks"} />
      ...
      <h2 className={styles.header}>小説・エッセイ</h2>
      {/*新着*/}
      ...
      <div className={styles.booksBox}>
        <h2>高評価の多い作品</h2>
        <div className={booksEvaluationPosts !== null ? styles.booksCardArea : styles.cardError}>
  ...
  while (newBooksData == undefined && popularBooksData == undefined && evaluationBooksData == undefined) {
    if (newBooksData == undefined) {
      ...
      const fetchNewComic = await fetch(
        "https://app.rakuten.co.jp/services/api/BooksTotal/Search/20170404?applicationId=1030475744401461181&booksGenreId=001004&sort=-releaseDate&hits=7"
      );
    ...
    if (popularBooksData == undefined) {
      ...
      const fetchPopularComic = await fetch(
        "https://app.rakuten.co.jp/services/api/BooksTotal/Search/20170404?applicationId=1030475744401461181&booksGenreId=001004&sort=sales&hits=7"
      );
    ...
    if (evaluationBooksData == undefined) {
      ...
      const fetchEvaluationComic = await fetch(
        "https://app.rakuten.co.jp/services/api/BooksTotal/Search/20170404?applicationId=1030475744401461181&booksGenreId=001004&sort=reviewAverage&hits=7"
      );
