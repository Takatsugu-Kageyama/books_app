...
const ComicPage = ({ newBooksData, popularBooksData, evaluationBooksData }: any) => {
  ...
  return (
    <div className={styles.overall}>
      <Head>
        <title>Book Talk ｜ 美容・暮らし・健康・料理</title>
        <meta name="viewport" content="width=device-width,initial-scale=1.0" />
        <meta name="description" content={"あなたの探したい本が見つかるBookTalk"} />
        <meta property="og:url" content={"https://booktalk.vercel.app/newBooks"} />
      ...
      <h2 className={styles.header}>新書</h2>
      {/*新着*/}
  ...
  while (newBooksData == undefined && popularBooksData == undefined && evaluationBooksData == undefined) {
    if (newBooksData == undefined) {
      ...
      const fetchNewComic = await fetch(
        "https://app.rakuten.co.jp/services/api/BooksTotal/Search/20170404?applicationId=1030475744401461181&booksGenreId=001020&sort=-releaseDate&hits=7"
      );
    ...
    if (popularBooksData == undefined) {
      ...
      const fetchPopularComic = await fetch(
        "https://app.rakuten.co.jp/services/api/BooksTotal/Search/20170404?applicationId=1030475744401461181&booksGenreId=001020&sort=sales&hits=7"
      );
    ...
    if (evaluationBooksData == undefined) {
      ...
      const fetchEvaluationComic = await fetch(
        "https://app.rakuten.co.jp/services/api/BooksTotal/Search/20170404?applicationId=1030475744401461181&booksGenreId=001020&sort=reviewAverage&hits=7"
      );
