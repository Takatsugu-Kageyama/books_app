...
const ComicPage = ({ newBooksData, popularBooksData, evaluationBooksData }: any) => {
  ...
  return (
    <div className={styles.overall}>
      ...
      <h2 className={styles.header}>ライトノベル</h2>
      {/*新着*/}
  ...
  while (newBooksData == undefined && popularBooksData == undefined && evaluationBooksData == undefined) {
    if (newBooksData == undefined) {
      ...
      const fetchNewComic = await fetch(
        "https://app.rakuten.co.jp/services/api/BooksTotal/Search/20170404?applicationId=1030475744401461181&booksGenreId=001017&sort=-releaseDate&hits=7"
      );
    ...
    if (popularBooksData == undefined) {
      ...
      const fetchPopularComic = await fetch(
        "https://app.rakuten.co.jp/services/api/BooksTotal/Search/20170404?applicationId=1030475744401461181&booksGenreId=001017&sort=sales&hits=7"
      );
    ...
    if (evaluationBooksData == undefined) {
      ...
      const fetchEvaluationComic = await fetch(
        "https://app.rakuten.co.jp/services/api/BooksTotal/Search/20170404?applicationId=1030475744401461181&booksGenreId=001017&sort=reviewAverage&hits=7"
      );
